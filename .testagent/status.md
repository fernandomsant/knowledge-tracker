...
- `Delete_removes_an_existing_link_and_reports_missing_links` verifies link deletion removes an existing connection and handles an unknown link safely.
- SQL migration execution passed through `npm run migrate`, including the conflict audit/backfill and both database triggers.
- `dotnet test src/KnowledgeTracker/KnowledgeTracker.Tests/KnowledgeTracker.Tests.csproj --no-restore` passed 8/8 tests.

...
public sealed class TopicsController(ITopicService topics) : ControllerBase
{
    ...
    [HttpDelete("{id:guid}")] public async Task<IActionResult> DeleteAsync(Guid id, CancellationToken ct) { try { return await topics.DeleteAsync(id, ct) ? NoContent() : NotFound(); } catch (InvalidOperationException error) { return Conflict(new ProblemDetails { Detail = error.Message }); } }
}

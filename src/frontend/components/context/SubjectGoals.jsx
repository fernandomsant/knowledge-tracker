...
function GoalProgress({ goal, notes, onSetSubGoalCompletion }) {
  ...
  if (goal.isCompleted || goal.currentOccurrenceCompletedAtUtc) return <div className="goal-visual"><GoalRing percent={100}/><div className="goal-visual-main"><div className="goal-meta"><span>Completion goal</span><strong>Done</strong></div><div className="goal-progress"><i style={{ width: '100%' }}/></div><div className="goal-insights"><span>{(goal.completedAtUtc ?? goal.currentOccurrenceCompletedAtUtc) ? `Completed ${dateFormatter.format(new Date(goal.completedAtUtc ?? goal.currentOccurrenceCompletedAtUtc))}` : 'Completed'}</span><b className="complete">Complete</b></div></div></div>;
...
export function SubjectGoals({ subjectId, goals, notes, includeDescendantNotes = false, topics = [], metricDefinitions, onCreateTopic, onRemoveTopic, onCreate, onUpdate, onRemove, onComplete, onSetSubGoalCompletion }) {
  const [open, setOpen] = useState(false);
  const [completingGoalId, setCompletingGoalId] = useState(null);
  const completingGoalIds = useRef(new Set());
  ...
  const complete = async goalId => {
    if (completingGoalIds.current.has(goalId)) return false;
    completingGoalIds.current.add(goalId);
    setCompletingGoalId(goalId);
    try {
      return await onComplete(goalId);
    } finally {
      completingGoalIds.current.delete(goalId);
      setCompletingGoalId(current => current === goalId ? null : current);
    }
  };
  return <section className="subject-goals"><div className="subject-goals-head"><div><span>GOALS</span><small>{includeDescendantNotes ? 'Includes progress from all child subjects.' : 'Track progress for this subject.'}</small></div><button type="button" className="text-button" onClick={beginCreating}><Plus size={15}/> Add goal</button></div>

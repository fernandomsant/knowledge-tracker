...
export function SubjectGoals({ goals, notes, metricDefinitions, onCreate, onRemove, onComplete }) {
  ...
  const save = async event => {
    ...
    const goal = { title, kind, metricDefinitionId: kind === 1 ? metricDefinitionId || null : null, targetValue: kind === 1 ? parseDecimal(targetValue) : null, targetDate: targetDate || null, period, periodStartDate: period === 4 ? periodStartDate || null : null, periodEndDate: period === 4 ? periodEndDate || null : null };
    if (!title.trim() || (kind === 1 && (!metricDefinitionId || !Number.isFinite(goal.targetValue) || goal.targetValue <= 0)) || (period === 4 && (!periodStartDate || !periodEndDate || periodStartDate > periodEndDate))) return;

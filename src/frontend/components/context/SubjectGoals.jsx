...
export function SubjectGoals({ goals, notes, metricDefinitions, onCreate, onRemove, onComplete, onSetSubGoalCompletion }) {
  ...
  const save = async event => {
    ...
    const goal = { title, kind, metricDefinitionId: kind === 1 ? metricDefinitionId || null : null, targetValue: kind === 1 ? parseDecimal(targetValue) : null, targetDate: targetDate || null, period, periodStartDate: period === 4 ? periodStartDate || null : null, periodEndDate: period === 4 ? periodEndDate || null : null, subGoals: kind === 2 ? subGoals.map(subGoal => subGoal.trim()).filter(Boolean) : [] };

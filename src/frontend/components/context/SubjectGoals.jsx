...
function GoalDayRegistry({ goal, onRecordDay }) {
  const [occurredOn, setOccurredOn] = useState(todayInputValue);
  const [isCompleted, setIsCompleted] = useState(false);
  const records = goal.dayRecords ?? [];
  const changeDate = value => {
    setOccurredOn(value);
    setIsCompleted(records.find(record => record.occurredOn === value)?.isCompleted ?? false);
  };
  const save = async event => {
    event.preventDefault();
    await onRecordDay(goal.id, occurredOn, isCompleted);
  };
  return <section className="goal-day-registry"><div><span>DAY REGISTRY</span><small>Record whether this goal was concluded on a specific day.</small></div><form onSubmit={save}><label>Date<input type="date" value={occurredOn} max={todayInputValue()} onChange={event => changeDate(event.target.value)}/></label><label className="goal-day-status"><input type="checkbox" checked={isCompleted} onChange={event => setIsCompleted(event.target.checked)}/> Completed</label><button className="text-button">Save day</button></form>{records.length ? <ol>{records.slice(0, 7).map(record => <li key={record.occurredOn}><time>{dateFormatter.format(new Date(`${record.occurredOn}T00:00:00`))}</time><b className={record.isCompleted ? 'complete' : 'not-complete'}>{record.isCompleted ? 'Completed' : 'Not completed'}</b></li>)}</ol> : <p>No days recorded yet.</p>}</section>;
}

export function SubjectGoals({ subjectId, goals, notes, topics = [], metricDefinitions, onCreateTopic, onRemoveTopic, onCreate, onUpdate, onRecordDay, onRemove, onComplete, onSetSubGoalCompletion }) {

...
function toNote(note) {
  return { id: note.id, subjectId: note.subjectId, title: note.title, excerpt: note.content, metrics: note.metrics ?? [], studyStartedAtUtc: note.studyStartedAtUtc, status: 'Draft', date: noteDateFormatter.format(new Date(note.studyStartedAtUtc)) };
}
...
export function useKnowledgeStore(accessToken) {
  ...
  const addNote = useCallback(async (subjectId, title, excerpt, studyStartedAtUtc, metrics) => {
    try {
      const note = await knowledgeClient.createStudyNote(accessToken, subjectId, title, excerpt, studyStartedAtUtc, metrics);
  ...
  const updateNote = useCallback(async (id, title, excerpt, studyStartedAtUtc, metrics) => {
    try {
      const note = await knowledgeClient.updateStudyNote(accessToken, id, title, excerpt, studyStartedAtUtc, metrics);

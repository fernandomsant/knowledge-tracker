...
public sealed record SubjectGoalDayRecordDetails(Guid Id, DateOnly OccurredOn, bool IsCompleted, DateTimeOffset RecordedAtUtc);
public sealed record SubjectGoalDetails(Guid Id, Guid SubjectId, Guid TopicId, string Title, GoalKind Kind, StudyMetricDefinitionDetails? MetricDefinition, decimal? TargetValue, decimal? CurrentValue, DateOnly? TargetDate, GoalPeriod Period, DateOnly? PeriodStartDate, DateOnly? PeriodEndDate, long PriorityPosition, bool IsCompleted, DateTimeOffset? CompletedAtUtc, DateTimeOffset CreatedAtUtc, IReadOnlyCollection<SubjectSubGoalDetails> SubGoals, IReadOnlyCollection<SubjectGoalDayRecordDetails> DayRecords);
